...
多人共享部署中的 `config.json` 必须由 root 所有且不可被 group/other 写入，例如 `root:gpuusers 0644`；单用户配置也可由当前 UID 所有。程序拒绝 symlink、非普通文件、超过 1 MiB 的配置以及不可信权限，避免共享目录中的其他用户注入 allocator 命令或私有日志路径。
...
首个预约写事务会把 GPU 数量、shared limit、5 分钟粒度、显存策略以及文件/目录模式绑定进台账。后续客户端若通过环境变量或不同配置改变这些关键规则，所有规划和写入都会 fail closed，并明确列出不一致字段；这避免同一个共享台账被不同用户按不同容量规则解释。旧台账在下一次成功新增或编辑时自动绑定当前策略。
...
共享台账、锁、WAL、备份与监控日志的叶子路径只接受普通文件；符号链接、FIFO、设备文件和符号链接备份目录会 fail closed。该防护避免共享目录成员将另一个用户执行 `bk` 时的写入重定向到其个人文件。
...
r 立即刷新（界面本身每秒自动刷新）
n 返回包含 NOW 标记的实时窗口
c 在 dark/light 配色间切换（仅当前 TUI 会话）
+/- 缩放时间轴，默认 5 分钟/列
←/→ 向过去或未来平移时间窗口，过去范围受台账保留期限制
↑/↓ 选择自己的预约
Tab 在预约列表与 GPU 列表之间切换焦点
a 新增预约
e 修改选中的预约
d 删除选中的预约
? 查看分区、彩色、可翻页的帮助和 Quick Tour
```
...
- 已经开始的预约不可再编辑；Edit 预览也不能提交到过去，核心调度层会再次执行同样校验，避免 CLI、TUI 或 Agent 绕过限制。
...
- 默认每列代表 5 分钟，并保留 NOW 之前 6 列作为近期上下文；亮色竖线明确标出当前时间，已经过去的区间会弱化显示。
- `←/→` 可浏览保留期内的历史预约或未来时段，`n` 一键回到实时窗口。历史预约只读；真实 GPU 利用率历史使用 `bk usage --rollups` 查看。
- 时间刻度分为日期、小时、分钟和连续刻度尺四行；左侧始终显示当前窗口的日期与星期，例如 `07-11 Fri`，跨午夜会在准确列标出下一天。小时显示为 `17h`，分钟显示为 `15` / `30`，整点不再重复显示 `00`。
- 每张 GPU 的共享容量、利用率和可分配空闲显存使用固定宽度字段，例如 `GPU0 S2/2 U72% F92.0G`；`F` 表示 free VRAM，容量变化不会再推挤后续指标，温度/进程/告警等附加指标只会整项显示，不会留下被截断的字符。
- TUI 同时提供深色和浅色主题。默认 `BK_TUI_THEME=auto`，优先根据终端的 `COLORFGBG` 判断背景；检测不准时可用 `BK_TUI_THEME=dark bk tui` 或 `BK_TUI_THEME=light bk tui` 固定主题，也可在界面内按 `c` 临时切换。普通文字跟随终端默认前景色，避免白底下浅灰不可读或黑底下深灰消失。
